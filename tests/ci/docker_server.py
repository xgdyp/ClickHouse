...
def auto_release_type(release_type: str, version: str) -> str:
    if release_type != "auto":
        return release_type
    validate_version(version)
    parts = version.split(".")
    current_version = ClickHouseVersion(parts[0], parts[1], parts[2], -1, git, parts[3])

    git_versions = get_tagged_versions()
    for i in reversed(range(len(git_versions))):
        if git_versions[i] < current_version:
            if i == len(git_versions):
                return "latest"
    ...
    raise ValueError(
        ...
        f"{current_version} < {reference_version}"
    )
